/**
 * Brand icon for a known external (OIDC) provider, matched on scheme/display
 * name. Falls back to no icon for providers we don't have a logo for, so a
 * future generic OIDC provider still renders a clean button. Shared by the
 * login and first-run onboarding screens.
 */
export function ProviderIcon({ scheme, displayName }: { scheme: string; displayName: string }) {
