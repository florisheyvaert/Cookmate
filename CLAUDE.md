...
- `System.Security.Cryptography.Xml` is pinned to `10.0.10` (bumped from 10.0.6 after advisories GHSA-23rf-6693-g89p / GHSA-8q5v-6pqq-x66h / GHSA-cvvh-rhrc-wg4q / GHSA-g8r8-53c2-pm3f / GHSA-mmjf-rqrv-855v flagged ≤10.0.9 — keep it at the latest 10.0.x runtime patch). `Microsoft.OpenApi` is explicitly pinned (2.11.0) though it's transitive via `Microsoft.AspNetCore.OpenApi`, to lift it past the 2.0.0 advisory GHSA-v5pm-xwqc-g5wc; stay on the 2.x line (ASP.NET OpenApi 10.x targets Microsoft.OpenApi v2). Because `NuGetAudit` runs as **warnings-as-errors**, any vulnerable (even transitive) package fails `dotnet restore` in CI — fix by bumping/pinning here, not by disabling the audit.
